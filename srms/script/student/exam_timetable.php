...
try {
	...
	$stmt = $conn->prepare("SELECT es.exam_date, es.start_time, es.end_time, es.room,
		...
		WHERE es.term_id = ? AND es.class_id = ?
		ORDER BY es.exam_date, es.start_time");
	$stmt->execute([$termId, (int)$class]);
...
<li><a class="app-menu__item active" href="student/exam_timetable"><i class="app-menu__icon feather icon-calendar"></i><span class="app-menu__label">Exam Timetable</span></a></li>
</ul>
</aside>
...
<div class="tile mb-3">
  ...
  <form class="row g-3" method="GET" action="student/exam_timetable">
	<div class="col-md-10">
...
<div class="tile">
  ...
  <div class="table-responsive">
	<table class="table table-hover table-striped">
	  ...
	  <tbody>
	  <?php if (count($rows) < 1) { ?>
		<tr><td colspan="5" class="text-muted">No entries found.</td></tr>
	  <?php } else { foreach ($rows as $r) { ?>
		<tr>
		  ...
		  <td><?php echo htmlspecialchars((string)($r['teacher_name'] ?? '')); ?></td>
