<?php
// CBC-ONLY: All legacy A/B/C/D, merit, division, and rank logic is now removed. Only CBC grading logic is active system-wide.
...
function app_exam_assessment_mode(PDO $conn, int $examId): string
{
	...
	return in_array($mode, ['cbc', 'consolidated'], true) ? $mode : 'normal';
}
...
{
	// Always return the canonical CBC-style grading bands.
...
function app_ensure_exam_grading_schema(PDO $conn): void
{
	...
	if (DBDriver === 'pgsql') {
		...
		if (!app_table_exists($conn, 'tbl_cbc_mark_submissions')) {
			$conn->exec("CREATE TABLE IF NOT EXISTS tbl_cbc_mark_submissions (
				id integer GENERATED BY DEFAULT AS IDENTITY NOT NULL,
				term_id integer NOT NULL,
				class_id integer NOT NULL,
				subject_id integer NOT NULL,
		...
		$conn->exec("CREATE UNIQUE INDEX IF NOT EXISTS tbl_cbc_mark_submissions_uq ON tbl_cbc_mark_submissions (term_id, class_id, subject_combination_id)");
		if (!app_table_exists($conn, 'tbl_teacher_assignments')) {
		...
		if (!app_table_exists($conn, 'tbl_cbc_mark_submissions')) {
			$conn->exec("CREATE TABLE IF NOT EXISTS tbl_cbc_mark_submissions (
				...
				review_note varchar(255) NULL,
				PRIMARY KEY (id),
				UNIQUE KEY tbl_cbc_mark_submissions_uq (term_id, class_id, subject_combination_id),
				CONSTRAINT tbl_cbc_mark_submissions_term_fk FOREIGN KEY (term_id) REFERENCES tbl_terms (id) ON DELETE CASCADE,
				CONSTRAINT tbl_cbc_mark_submissions_class_fk FOREIGN KEY (class_id) REFERENCES tbl_classes (id) ON DELETE CASCADE,
				CONSTRAINT tbl_cbc_mark_submissions_subject_fk FOREIGN KEY (subject_id) REFERENCES tbl_subjects (id) ON DELETE CASCADE,
				CONSTRAINT tbl_cbc_mark_submissions_subjectcomb_fk FOREIGN KEY (subject_combination_id) REFERENCES tbl_subject_combinations (id) ON DELETE CASCADE,
	...
	$cbcRows = app_default_overall_grading_rows();
	...
	$stmt = $conn->prepare("SELECT id FROM tbl_grading_systems WHERE type = 'cbc' ORDER BY id ASC LIMIT 1");
	$stmt->execute();
	$cbcSystemId = (int)$stmt->fetchColumn();
	if ($cbcSystemId < 1) {
		if (DBDriver === 'pgsql') {
			$stmt = $conn->prepare("INSERT INTO tbl_grading_systems (name, type, description, is_default, is_active) VALUES (?,?,?,?,?) RETURNING id");
			$stmt->execute(['CBC Term System', 'cbc', 'Competency grading for CBC assessments', 0, 1]);
			$cbcSystemId = (int)$stmt->fetchColumn();
		} else {
			$stmt = $conn->prepare("INSERT INTO tbl_grading_systems (name, type, description, is_default, is_active) VALUES (?,?,?,?,?)");
			$stmt->execute(['CBC Term System', 'cbc', 'Competency grading for CBC assessments', 0, 1]);
			$cbcSystemId = (int)$conn->lastInsertId();
		}
	}

	$stmt = $conn->prepare("SELECT COUNT(*) FROM tbl_grading_scales WHERE grading_system_id = ?");
	$stmt->execute([$cbcSystemId]);
	if ((int)$stmt->fetchColumn() < 1) {
		$stmt = $conn->prepare("INSERT INTO tbl_grading_scales (grading_system_id, min_score, max_score, grade, points, remark, sort_order, is_active) VALUES (?,?,?,?,?,?,?,?)");
		foreach ($cbcRows as $row) {
			$stmt->execute([$cbcSystemId, $row['min'], $row['max'], $row['grade'], $row['points'], $row['remark'], $row['order'], $row['active']]);
		}
	}
...
function app_class_name_parts(string $name): array
{
	...
	$canonical = app_cbc_canonical_class_name($name);
	if ($canonical !== '') {
...
function app_build_class_name(string $grade, string $stream = '', string $fallback = ''): string
{
	...
	if ($grade === '' && $fallback !== '') {
		$canonicalFallback = app_cbc_canonical_class_name($fallback);
		return $canonicalFallback !== '' ? $canonicalFallback : ucfirst($fallback);
	}
	$canonicalGrade = app_cbc_canonical_class_name($grade);
	if ($canonicalGrade !== '') {
