...
try {
  ...
  $stmt = $conn->prepare("UPDATE tbl_cbe_mark_submissions SET status = 'approved', reviewed_at = CURRENT_TIMESTAMP, reviewed_by = ? WHERE id = ? AND status = 'submitted'");
  ...
  app_audit_log($conn, 'staff', (string)$account_id, 'cbe_marks.approve', 'submission', (string)$submissionId);
  $_SESSION['reply'] = array (array("success", "CBE marks approved."));
} catch (Throwable $e) {
