...
try {
  ...
  $meta = $conn->prepare("SELECT class_id, term_id FROM tbl_cbe_mark_submissions WHERE id = ? LIMIT 1");
  ...
  $stmt = $conn->prepare("UPDATE tbl_cbe_mark_submissions SET status = 'rejected', reviewed_at = CURRENT_TIMESTAMP, reviewed_by = ?, review_note = ? WHERE id = ? AND status = 'submitted'");
  ...
  app_audit_log($conn, 'staff', (string)$account_id, 'cbe_marks.reject', 'submission', (string)$submissionId);
  $_SESSION['reply'] = array (array("success", "CBE marks rejected."));
} catch (Throwable $e) {
