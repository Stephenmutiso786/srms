...
try {
  ...
  $stmt = $conn->prepare("UPDATE tbl_cbe_mark_submissions SET status = 'draft', reviewed_at = CURRENT_TIMESTAMP, reviewed_by = ? WHERE id = ? AND status = 'approved'");
  ...
  app_audit_log($conn, 'staff', (string)$account_id, 'cbe_marks.unlock', 'submission', (string)$submissionId);
  $_SESSION['reply'] = array (array("success", "CBE marks unlocked to draft."));
} catch (Throwable $e) {
