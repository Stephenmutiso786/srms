...
try {
    ...
    $del = $conn->prepare('DELETE FROM tbl_role_module_allocations WHERE role_id = ? AND portal = ?');
    $del->execute([$roleId, $portal]);

    if (!empty($filtered)) {
        $isPgsql = (defined('DBDriver') && DBDriver === 'pgsql');
        if ($isPgsql) {
            $ins = $conn->prepare('INSERT INTO tbl_role_module_allocations (role_id, portal, module_key) VALUES (?, ?, ?) ON CONFLICT DO NOTHING');
        } else {
            $ins = $conn->prepare('INSERT IGNORE INTO tbl_role_module_allocations (role_id, portal, module_key) VALUES (?, ?, ?)');
        }
        foreach (array_keys($filtered) as $k) {
            $ins->execute([$roleId, $portal, $k]);
        }
    }
