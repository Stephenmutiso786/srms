...
app_require_permission('communication.send', '../sms_topup');
...
try {
	...
	$description = 'Teacher SMS token purchase';
	...
	$stmt->execute([$phone, $tokens, $amount, $accountRef, 'pending', (int)$account_id]);
	...
	app_audit_log($conn, 'staff', (string)$account_id, 'sms.wallet.teacher_topup', 'wallet', '1', [
		'tokens' => $tokens,
		'phone' => $phone,
		'amount' => $amount,
	]);
