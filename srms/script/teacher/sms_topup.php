...
app_require_permission('communication.send', 'teacher/sms');
app_require_unlocked('communication', 'teacher/sms');
...
try {
	...
	if (app_table_exists($conn, 'tbl_sms_topup_requests')) {
		$stmt = $conn->prepare("SELECT phone, tokens, amount, status, customer_message, created_at, updated_at FROM tbl_sms_topup_requests WHERE created_by = ? ORDER BY id DESC LIMIT 10");
...
<div><a class="btn btn-outline-secondary" href="teacher/sms"><i class="bi bi-arrow-left me-1"></i>Back to SMS</a></div>
</div>
<?php if ($error !== '') { ?>
...
<div class="col-12"><div class="border rounded p-3"><div class="text-muted small">Rule</div><div class="fw-bold">1 SMS token = 1 SMS segment</div></div></div>
</div>
</div>
<div class="tile">
<h3 class="tile-title">Purchase Tokens</h3>
<form class="row g-3" method="POST" action="teacher/core/mpesa_sms_topup" autocomplete="off">
<div class="col-md-12"><label class="form-label">Phone Number</label><input class="form-control" name="phone" placeholder="2547XXXXXXXX" required></div>
<div class="col-md-6"><label class="form-label">Tokens</label><input class="form-control" type="number" min="1" step="1" name="tokens" value="100" required></div>
<div class="col-md-6"><label class="form-label">Rate per Token</label><input class="form-control" value="1" disabled></div>
<div class="col-12 d-grid"><button class="btn btn-primary" type="submit"><i class="bi bi-phone-vibrate me-1"></i>Buy Tokens</button></div>
<p class="text-muted mb-0">M-Pesa will prompt the PIN on the entered phone. The system never collects or stores the PIN.</p>
</form>
</div>
</div>
<div class="col-lg-7">
<div class="tile">
<h3 class="tile-title">My Recent Token Purchases</h3>
