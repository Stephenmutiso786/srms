...
SET @dbdriver := (SELECT DATABASE());
SET @hasColumn := (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'tbl_certificates' AND COLUMN_NAME = 'mean_score');
ALTER TABLE tbl_certificates ADD COLUMN IF NOT EXISTS mean_score DECIMAL(5,2) DEFAULT NULL;
ALTER TABLE tbl_certificates ADD COLUMN IF NOT EXISTS merit_grade VARCHAR(1) DEFAULT NULL;
ALTER TABLE tbl_certificates ADD COLUMN IF NOT EXISTS competencies_json LONGTEXT DEFAULT NULL;
ALTER TABLE tbl_certificates ADD COLUMN IF NOT EXISTS certificate_category VARCHAR(50) DEFAULT 'general';
ALTER TABLE tbl_certificates ADD COLUMN IF NOT EXISTS position_in_class INT DEFAULT NULL;
ALTER TABLE tbl_certificates ADD COLUMN IF NOT EXISTS approved_by INT DEFAULT NULL;
...
CREATE TABLE IF NOT EXISTS tbl_promotion_batches (
    ...
    created_by INT NOT NULL REFERENCES tbl_staff(id) ON DELETE SET NULL,
...
INSERT IGNORE INTO tbl_cbc_competencies (school_id, competency_name, competency_code, description, strand, grade_range, status)
VALUES
    (NULL, 'Communication and Collaboration', 'CC-001', 'Ability to communicate effectively and collaborate with others', 'Core Competency', 'G1-G6', 'active'),
    (NULL, 'Critical Thinking and Problem Solving', 'CTPS-001', 'Ability to analyze problems and find creative solutions', 'Core Competency', 'G1-G6', 'active'),
    (NULL, 'Creativity and Imagination', 'CI-001', 'Ability to think creatively and generate new ideas', 'Core Competency', 'G1-G6', 'active'),
    (NULL, 'Citizenship and Personal Development', 'CPD-001', 'Understanding of civic responsibilities and personal values', 'Core Competency', 'G1-G6', 'active'),
    (NULL, 'Digital Literacy', 'DL-001', 'Proficiency with digital tools and technologies', 'Core Competency', 'G1-G6', 'active'),
    (NULL, 'Learning Outcomes Achievement', 'LOA-001', 'Achievement of subject-specific learning outcomes', 'Academic', 'G1-G6', 'active');

-- ============================================================================
-- SEED: Default Promotion Rules
-- ============================================================================

INSERT IGNORE INTO tbl_promotion_rules (school_id, grade_level, min_score_for_promotion, require_fees_clearance, require_report_finalization, require_headteacher_approval, auto_generate_certificate, certificate_type)
VALUES
    (NULL, 1, 40.0, TRUE, TRUE, TRUE, FALSE, 'general'),
    (NULL, 2, 40.0, TRUE, TRUE, TRUE, FALSE, 'general'),
    (NULL, 3, 40.0, TRUE, TRUE, TRUE, FALSE, 'general'),
    (NULL, 4, 40.0, TRUE, TRUE, TRUE, FALSE, 'general'),
    (NULL, 5, 40.0, TRUE, TRUE, TRUE, FALSE, 'general'),
    (NULL, 6, 40.0, TRUE, TRUE, TRUE, TRUE, 'primary_completion'),
    (NULL, 7, 40.0, TRUE, TRUE, TRUE, FALSE, 'general'),
    (NULL, 8, 40.0, TRUE, TRUE, TRUE, FALSE, 'general'),
    (NULL, 9, 40.0, TRUE, TRUE, TRUE, TRUE, 'junior_completion');
